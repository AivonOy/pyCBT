...
def DF1plus(n,V,C_sigma):
    return (e/2.0)*(fii(n,C_sigma)+fii(n+1,C_sigma)-(V))

def DF1minus(n,V,C_sigma):
    return (e/2.0)*(V-(fii(n,C_sigma)+fii(n-1,C_sigma)))

def DF2plus(n,V,C_sigma):
    return (e/2.0)*(-V-(fii(n,C_sigma)+fii(n-1,C_sigma)))

def DF2minus(n,V,C_sigma):
    return (e/2.0)*((fii(n,C_sigma)+fii(n+1,C_sigma))+V)

def gamma1plus(n,V,T,R_T,C_sigma):
...
def gamma1minus(n,V,T,R_T,C_sigma):
...
def gamma2plus(n,V,T,R_T,C_sigma):
...
def gamma2minus(n,V,T,R_T,C_sigma):
...
def calc_current_1(V,T,R_T,C_sigma):
    """
...
# tests
def calc_current(V,T,R_T,C_sigma):
    """
    calculates current when voltage V is over 2 (two) junctions
    """
    sigma=array([1.0])
