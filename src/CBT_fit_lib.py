...
class CBT_fitter():
    ...
    def __init__(self,filename="data/cbt14.txt", T_init=50e-3, island_size_init=1.0,
                 R_tunnel_init=30e3,TEC_init=100e-3,parallel_arrays=1.0, n_max=200, N=2.0,
        ...
        self.R_tunnel_init = R_tunnel_init
        self.T_init = T_init
        ...
        p0=[1.0/(R_tunnel_init),T_init,TEC_init]
        meas_R = meas_R[indices]
        # initial optimization
        self.plsq=plsq = optimize.fmin_bfgs(self.residuals1, p0, args=(meas_G,meas_V),
                                                gtol=1e-6, full_output=1,maxiter=3e2,
                                                callback=self.call_func)          
        print "==== After initial optimization: ===="     
        print "R_T = %g"%(1.0/(plsq[0][0]*N))
        print "T = %g mK"%(plsq[0][1]*1000.0)
        print "Ec = %g mK"%(plsq[0][2]*(N-1.0)/N*1000)
        print "Csigma = %g fF"%(e**2/(plsq[0][2]*k)*1e15)
        #v_offset = plsq[0][3]*1e6 #uV
        """
        self.v_offset =0.0
        #print "V offset = %g uV"%(v_offset)
        #self.meas_V=self.meas_V-v_offset*1e-6
        #if show_first_fit==True:
        #    self.plot_result_initial()
        """
        # main optimization
        R_T_init = (1.0/(plsq[0][0]*N))*1e-3
        C_sigma_init=e**2/(plsq[0][2]*k)*1e15*1.0 #for some reason x4
        T_p_init = plsq[0][1]*1e3
        island_volume_init = island_size_init
        x1=[R_T_init,C_sigma_init,T_p_init,island_volume_init]
        x1=[R_T_init,C_sigma_init,T_p_init]
        self.plot_start(x1)
        #print "Initial values for main optimization"
        #print x1
        if bounds == None:
            ...
            self.xopt1 = optimize.fmin_l_bfgs_b(self.optimize_1, x1, factr=1e7, approx_grad=True, bounds=bounds,disp=1)
        toc = time.clock()
        print "==== After main optimization: ===="     
        print "R_T = %g"%(self.xopt1[0][0])
        print "T = %g mK"%(self.xopt1[0][2])
        print "C_sigma = %g "%(self.xopt1[0][1])
        #print "Island volume = %g um3"%(self.xopt1[0][3])
        """
        #print "Calculation took %g seconds"%(toc-tic)
        #print "Calculation took %g minutes"%((toc-tic)/60.0)
    ...
    def fit_full_curve(self):
        ...
        if self.classic_curve_fitted==True:
            R_T_init = (1.0/(self.plsq[0][0]*N))*1e-3
    ...
    def G_optimize_func(self,x,V):
        ...
        T_opt = x[1]
        EC_opt = x[2]
        return Gt_opt*(1.0-2*(N-1.0)/N*EC_opt/T_opt*g(e*V/(N*k*T_opt)))
    ...
    def optimize_1(self,x):
        ...
        for V in self.meas_V:
            ...
            G.append(calc_G(self.sigma,self.N,V,R_T,C_sigma,T_p,self.island_size,self.const_P))
